...
# defined_twice = ["bert_config_file","input_file","output_dir","init_checkpoint","max_seq_length","max_predictions_per_seq",
# "recurring_span_selection","only_recurring_span_selection","max_questions_per_seq","do_train","do_eval",
# "train_batch_size","eval_batch_size","learning_rate","num_train_steps","num_warmup_steps","save_checkpoints_steps",
# "keep_checkpoint_max","iterations_per_loop","max_eval_steps","use_tpu","tpu_name","tpu_zone","gcp_project","master","num_tpu_cores"]
...
flags.DEFINE_string(
    "input_file", None,
    "Input TF example files (can be a glob or comma separated).",allow_override=True)
...
flags.DEFINE_integer(
    ...
    "than this will be padded. Must match data generation.",allow_override=True)

flags.DEFINE_integer(
    "max_predictions_per_seq", 80,
    "Maximum number of masked LM predictions per sequence. "
    "Must match data generation.",allow_override=True)

flags.DEFINE_bool("recurring_span_selection", True, "Whether to mask recurring spans",allow_override=True)
flags.DEFINE_bool("only_recurring_span_selection", True, "If set to true, MLM is not applied",allow_override=True)
flags.DEFINE_integer("max_questions_per_seq", 30, "",allow_override=True)

flags.DEFINE_bool("do_train", False, "Whether to run training.",allow_override=True)

flags.DEFINE_bool("do_eval", False, "Whether to run eval on the dev set.",allow_override=True)

flags.DEFINE_integer("train_batch_size", 256, "Total batch size for training.",allow_override=True)

flags.DEFINE_integer("eval_batch_size", 8, "Total batch size for eval.",allow_override=True)

flags.DEFINE_float("learning_rate", 1e-4, "The initial learning rate for Adam.",allow_override=True)

flags.DEFINE_integer("num_train_steps", 1000000, "Number of training steps.",allow_override=True)

flags.DEFINE_integer("num_warmup_steps", 10000, "Number of warmup steps.",allow_override=True)

flags.DEFINE_integer("save_checkpoints_steps", 10000,
                     "How often to save the model checkpoint.",allow_override=True)

flags.DEFINE_integer("keep_checkpoint_max", 100, "",allow_override=True)

flags.DEFINE_integer("iterations_per_loop", 1000,
                     "How many steps to make in each estimator call.",allow_override=True)

flags.DEFINE_integer("max_eval_steps", 100, "Maximum number of eval steps.",allow_override=True)

flags.DEFINE_bool("use_tpu", False, "Whether to use TPU or GPU/CPU.",allow_override=True)

tf.flags.DEFINE_string(
    "tpu_name", None,
    "The Cloud TPU to use for training. This should be either the name "
    "used when creating the Cloud TPU, or a grpc://ip.address.of.tpu:8470 "
    "url.",allow_override=True)
...
tf.flags.DEFINE_string("master", None, "[Optional] TensorFlow master URL.",allow_override=True)

flags.DEFINE_integer(
    "num_tpu_cores", 8,
    "Only used if `use_tpu` is True. Total number of TPU cores to use.",allow_override=True)
