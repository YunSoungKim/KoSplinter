...
from evaluate_korquad import eval_during_train
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    official_eval_results = eval_during_train(args, step=prefix)
...
def main():
    ...
    if args.qass_head:
        model = SplinterForQuestionAnswering.from_pretrained(args.model_name_or_path, config=config,
                                                            cache_dir=args.cache_dir if args.cache_dir else None)
    else:
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.qass_head:
            model = SplinterForQuestionAnswering.from_pretrained(args.output_dir, config=config,
                                                      cache_dir=args.cache_dir if args.cache_dir else None)
        else:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if args.qass_head:
                model = SplinterForQuestionAnswering.from_pretrained(checkpoint, config=config,
                                                          cache_dir=args.cache_dir if args.cache_dir else None)
            else:
